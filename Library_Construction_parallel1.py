...
def process_data_threaded_front(m):
	d={}
	...
	for i in d.keys():
		d[i].join()
def process_data_threaded_end(m):
	d={}
	for i in range(len(m)):
		d["t"+str(i)]=threading.Thread(target=library_construction_end,args=(m[i],i))
	for i in d.keys():
		d[i].start()
	for i in d.keys():
		d[i].join()
#process threads for front library construction
process_data_threaded_front(util.modules)
...
#process threads for end library construction
process_data_threaded_end(util.modules)
